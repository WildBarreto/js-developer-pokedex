...
async function loadPokemonItens(offset, limit) {
    ...
    for (const pokemon of pokemons) {
        const liHtml = `
        <li id="${pokemon.number}"class="pokemon item-button ${pokemon.type
            } hover">
  ...
  `;
    ...
    buttons.forEach((button) => {
        button.addEventListener("click", function () {
            ...
            const modalHtml =
                `<li id="${selectPokemon.number}"class="pokemon item-button ${selectPokemon.type}">
                <span class="number">#${selectPokemon.number}</span>
                <span class="name">${selectPokemon.name}</span>
                <img src="${selectPokemon.photo}" alt="${selectPokemon.name}">

                <div class="modalDetail">           
                    <ol class="abilities">
                    <span class="titleAbilities">Habilidades:</span>
                    ${selectPokemon.abilities.map((ability) => `<li class="abilities${ability}">${ability}</li>`)
                    .join("")}
                    </ol>
                    <ol class="statsName">
                        <span class ="titleStats">Status:</span>
                         ${selectPokemon.stat.map((nameStats) => `<li class="statsName${nameStats}">${nameStats}</li>`)
                    .join("")}
                    </ol>
                    <ol class="stats">
